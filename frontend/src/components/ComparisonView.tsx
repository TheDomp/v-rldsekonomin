...
export const ComparisonView: React.FC<ComparisonViewProps> = ({ countries, onClose }) => {
    ...
    const pillarData = [
        { name: 'Likviditet', ...countries.reduce((acc, c) => ({ ...acc, [c.name]: c.pillars.liquidity }), {}) },
        { name: 'Burn Rate', ...countries.reduce((acc, c) => ({ ...acc, [c.name]: c.pillars.burnRate }), {}) },
        { name: 'Skuldstruktur', ...countries.reduce((acc, c) => ({ ...acc, [c.name]: c.pillars.debtStructure }), {}) },
        { name: 'Tillväxt', ...countries.reduce((acc, c) => ({ ...acc, [c.name]: c.pillars.realGrowth }), {}) },
        { name: 'Demografi', ...countries.reduce((acc, c) => ({ ...acc, [c.name]: c.pillars.demographics }), {}) },
    ];
    ...
    return (
        ...
        >
            <div className="flex justify-between items-center mb-8">
                <div>
                    <h2 className="text-2xl font-bold text-white">Jämförelseanalys</h2>
                    <p className="text-sm text-[var(--color-eco-text-muted)]">Sida-vid-sida analys av {countries.length} valda ekonomier</p>
                </div>
                <button
                    onClick={onClose}
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={countries}>
                            ...
                            <Tooltip
                                ...
                                contentStyle={{ backgroundColor: '#1e293b', border: '1px solid #334155', borderRadius: '8px' }}
                            />
                            <Bar dataKey="healthIndex" name="Hälsoindex" radius={[4, 4, 0, 0]}>
                ...
                <table className="w-full text-left border-collapse">
                    <thead>
                        <tr className="border-b border-slate-700">
                            <th className="py-4 px-4 text-slate-400 font-medium whitespace-nowrap">Mätvärde</th>
                            {countries.map(c => (
